...
> 마지막 업데이트: 2026-05-27 (PDF 5편 + OpenAlex 635편 + 응용 분야 682편 기반, arXiv 제외)
...
## Part 3: 응용 분야 논문 (키워드 수집 682편, arXiv 제외) 기반 인사이트

> `fetch_applied.py`로 수집한 "social network" / "text analysis" 키워드 상위 인용 논문 (arXiv 제외). SNA·텍스트마이닝이 마케팅·보건·법률 등 응용 분야에서 어떻게 쓰이는지, 신규 고객 세그먼트를 파악하기 위한 목적.
> 분석일: 2026-05-27 / 커버: 682편 (2026년 451편 + 2025년 231편, 100%)

→ 상세 분석: [[pages/insights/applied_method_frequency_2026|응용 분야 방법론 빈도]] · [[pages/insights/applied_domain_venue_2026|도메인 및 학술지 분포]]

### 방법론 순위 (응용 분야, 682편 기준)

| 순위 | 방법론 | 편수 | NetMiner 지원 |
|------|--------|------|:-------------:|
| 1 | 감성 분석 (BERT·VADER·TextBlob·ABSA) | ~253 (37%) | ⚠️ 부분 |
| 2 | LLM/GPT 활용 (파인튜닝·RAG·에이전트) | ~234 (34%) | ❌ |
| 3 | 토픽 모델링 (LDA·BERTopic·STM·keyATM) | ~144 (21%) | ✅ |
| 4 | 텍스트 분류 / ML (SVM·RF·트랜스포머) | ~136 (20%) | ⚠️ 부분 |
| 5 | SNA / 네트워크 분석 | ~75 (11%) | ✅ |
| 6 | 딥러닝 (CNN·LSTM·GRU) | ~50 (7%) | ❌ |
| 7 | GNN | ~34 (5%) | ❌ |
| 8 | 멀티모달 분석 | ~25 (4%) | ❌ |
...
**1. 감성 분석이 응용 분야 1위 (682편 기준)**
- 682편 중 ~37%(~253편)가 감성 분석. 2025년 신규 논문에서 46%로 더 높은 비중.
- ABSA(측면 기반)·저자원 언어·감정 다중분류로 고도화. NetMiner의 기본 감성 분석으로는 커버 부족 → ABSA 강화 필요.

**2. LLM 경량화 + 오픈소스 전환**
- LoRA/PEFT로 소비자급 GPU에서 LLM 파인튜닝 가능해짐. 오픈소스(Llama, Mistral)가 GPT와 경쟁.
- NetMiner 포지셔닝: "LLM 전처리 결과물 → NetMiner SNA/토픽 분석" 후단 도구.

**3. BERTopic·STM·keyATM 세분화**
- BERTopic(~34편) 성장, STM(~15편) 부상, keyATM(~8편) 신흥. LDA만으로는 다양한 토픽모델링 수요 대응 불가.
- NetMiner의 LDA 지원은 경쟁력 있으나 BERTopic 중기 도입 필요.

**4. SNA + 텍스트 결합이 핵심 차별화 포인트**
- 텍스트+SNA 결합 ~30편. 서지계량, 정책담론, K-POP, 재난안전 등 분야 확산.
- **NetMiner의 유일한 통합 포지션** — GUI 기반 SNA+텍스트 통합 도구는 NetMiner뿐.

**5. Textom 경쟁 심화**
- 682편 중 Textom 직접 언급(2026_yoo K-POP 연구), NetMiner 추정 사용(2025_lee 한국 법률 담론). 국내 한국어 의미연결망 시장에서 직접 경쟁.

**6. 저자원 언어 + 글로벌 연구자 수요**
- 인도네시아·아랍어·방글라어 등 비영어 논문이 전체 20% 이상. 다국어 텍스트 분석 수요 증가 — NetMiner 한국어 특화에서 다국어 지원으로 확장 기회.
