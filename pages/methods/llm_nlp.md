...
사전학습 대규모 언어모델(LLM)을 연구 인프라로 활용하는 방법론 범주. 응용 분야 논문 682편 기준 **2위(~234편, ~34%)**. 초기에는 LLM 자체가 연구 대상이었으나, 현재는 기존 인간 코딩·분류 작업을 대체하는 "연구 인프라"로 전환 완료. 2025년 신규 논문에서 LoRA/PEFT 경량 파인튜닝과 오픈소스 LLM 활용이 급증.
...
| **RAG** (검색 증강 생성) | ~15 | 법률 RAG, 의료 RAG, GraphRAG·BifrostRAG (지식그래프 결합) |
| **LoRA / PEFT** (경량 파인튜닝) | ~10 | Llama-3.1 LoRA, QLoRA, 4-bit 양자화 — 소비자급 GPU 대응 |
| **LLM 에이전트 시스템** | ~9 | 멀티에이전트 토론, 소셜 시뮬레이션, 오케스트레이션 |
...
| **LLaMA / Mistral / Qwen / DeepSeek** | 오픈소스 LLM 파인튜닝·벤치마킹 | ~35 |
...
## 관련 트렌드 (682편 기준, 2025년 업데이트)

- **LoRA/PEFT 경량 파인튜닝 표준화**: 소비자급 GPU에서 LLM 파인튜닝 가능. "오픈소스 LLM + LoRA"가 도메인 특화 모델의 새 표준 (2025년 신규 다수 확인)
- **RAG + 지식그래프 융합**: GraphRAG·BifrostRAG 등 단순 RAG를 넘어 구조화 지식그래프 결합 아키텍처 등장. 법률·의료·보안 도메인 중심
- **LLM 에이전트 사회과학 진입**: LLM 에이전트로 소셜미디어 양극화 시뮬레이션, 네트워크 형성 비교 — 선행 트렌드 포착 (2025_boji, 2025_papachrist)
- **저자원 언어 NLP 급증**: 인도네시아어·아랍어·방글라어·카자흐어 등 비영어 연구 20% 이상
- **LLM vs 인간 코더 비교 연구** 급증 — 방법론적 타당성 검증이 주요 연구 주제
- **설명 가능 AI(XAI) 내재화**: SHAP·LIME·LIME 결합이 의료·법률 분야 표준으로 진입
